...
async function querySQL(ctx: QueryContext, sql: string): Promise<GatewayResult> {
  ...
  // Failover: localhost dulu, lalu 10.0.0.110 (kecuali override allowlist).
  const gatewayBases = resolveSqlGatewayCandidates({ override: gatewayBaseStorage.getStore() })
  let lastError: GatewayResult | null = null
  for (const base of gatewayBases) {
    try {
      ...
      if (!response.ok || result.success === false) {
        ...
        // Base gateway gagal — coba kandidat berikutnya.
        continue
      }
      ...
      // Koneksi gagal — coba base gateway berikutnya.
      lastError = {
        success: false,
  ...
  return lastError ?? { success: false, error: 'SQL Gateway tidak tersedia.' }
}
