...
You (an agent) are working inside `Module Services/rjfm`, one module of
...
| Mode | URL | Auth |
|---|---|---|
| **Direct** (independent) | `http://localhost:8011` | This module verifies the shared RS256 cookie itself (`keys/public.pem` at repo root) or its own login page |
| **Via gateway proxy** | `http://localhost:3001/rjfm` | The Bun gateway (`server_bun.js`) already verified the cookie and forwards identity headers: `X-User-Id`, `X-User-Name`, `X-User-Email`, `X-User-Role` |
...
- This module runs on ITS OWN PORT and must start with zero other processes.
  ...
  independently (`node scripts/module.js start rjfm` from repo root).
