...
export function buildExecutiveDivisionRows({ breakdown = [], efficiency = [] } = {}) {
  ...
  return sourceRows
    .map((row) => {
      ...
      // [HK-KEHADIRAN] divisor HK hanya pakai hari kerja efektif / hari_kerja (kehadiran)
      const totalHk = toNumber(efficiencyRow.total_hk_efektif ?? efficiencyRow.total_man_days ?? efficiencyRow.total_hk);
...
export function buildExecutiveTrendRows({ trends = [], productivityTrend = [] } = {}) {
  ...
  return (Array.isArray(trends) ? trends : [])
    ...
    .map((row) => {
      ...
      const totalWage = toNumber(row?.total_wage);
      const overtime = toNumber(row?.total_ot);
      const hkEfektif = toNumber(row?.total_hk_efektif ?? row?.total_hk ?? row?.total_man_days);
      const totalHk = hkEfektif;
