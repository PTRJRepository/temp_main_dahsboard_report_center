...
/** Produktivitas ton/HK. Null bila HK 0. Divisor HANYA HK kehadiran (total_hari_kerja / hari_kerja_efektif), bukan jumlah_hk. */
export function productivity(row) {
    const hk = Number(row?.total_hari_kerja ?? 0) || Number(row?.total_hk ?? 0) || 0;
    if (hk <= 0) return null;
    const tonase = Number(row?.total_tonase) || 0;
    return tonase / hk;
}
/** HK libur = HK total (jumlah_hk) − HK hadir (total_hari_kerja). JANGAN fallback total_hk ke hadir. */
export function hkLibur(row) {
    const total = Number(row?.total_hk ?? 0) || 0;
    const hadir = Number(row?.total_hari_kerja ?? 0) || 0;
    if (!total) return null;
    return Math.max(0, total - hadir);
}
/** Cost per HK — divisor HANYA HK kehadiran (total_hari_kerja). JANGAN total_hk. */
export function costPerHk(row) {
    const hk = Number(row?.total_hari_kerja ?? 0) || 0;
    if (hk <= 0) return null;
    const wage = Number(row?.total_wage ?? row?.total_upah_kotor ?? 0) || 0;
    return wage / hk;
}
