...
const normSnapRow = (r) => {
    // [COST/HK] Hari kerja efektif = jumlah_hk - cuti_minggu - cuti_nasional (rule bisnis)
    const cm = num(r.cuti_minggu_hari) || num(r.cuti_minggu) || 0;
    const cn = num(r.cuti_nasional_hari) || num(r.cuti_nasional) || 0;
    const hkEfektif = num(r.hari_kerja_efektif) || (num(r.jumlah_hk) > 0 ? Math.max(0, num(r.jumlah_hk) - cm - cn) : (num(r.hari_kerja) || 0));
    return {
        ...r,
        ...
        hari_kerja_efektif: hkEfektif,
        kehadiran: hkEfektif,
        cuti_minggu_hari: cm,
        cuti_nasional_hari: cn,
...
export default function SalaryAnalysisPage() {
    ...
    const [chartType, setChartType] = useState('ALL'); // Jenis pekerjaan untuk grafik: ALL | Panen | Perawatan | Transport
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                const dedupeByNik = (rows) => {
                    const m = new Map(); const out = [];
                    for (const r of rows) {
                        const nik = String(r.nik || r.actual_nik || '').trim().toUpperCase();
                        if (!nik) { out.push(r); continue; }
                        const idx = m.get(nik);
                        if (idx !== undefined) {
                            const ex = out[idx];
                            if (String(r.emp_code || '').toUpperCase() > String(ex.emp_code || '').toUpperCase()) out[idx] = r;
                        } else { m.set(nik, out.length); out.push(r); }
                    }
                    return out;
                };
                if (missing.length === 0) {
                    setGlobalRoster(dedupeByNik(snapRows));
                    return;
                }
                ...
                setGlobalRoster(dedupeByNik([...snapRows, ...live]));
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            // [RETAIN-CONSISTENCY] Dedupe per NIK di garis pertahanan kedua (backend sudah lakukan filter aktif + dedupe).
            // Dedupe NIK di frontend tetap diperlukan kalau backend skip enrichment (error) atau fallback live dipakai.
            // Dedupe key: NIK KTP (actual_nik||nik) bukan emp_name; beda nik tapi benar-benar orang berbeda → jangan di-drop.
            // Bila nik kosong, jatuh ke emp_code sebagai key unik (tidak didedupe antar nama).
            {
                const nikIdx = new Map();
                const deduped = [];
                const newerWins = (a, b) => {
                    // C-prefix → B → A terbaru; bila beda panjang, yang lebih panjang di belakang dianggap terbaru
                ...
                for (const r of rows) {
                    const nik = String(r.nik || r.actual_nik || r.NewICNo || '').trim().toUpperCase();
                    const key = nik || String(r.emp_code || '').trim().toUpperCase() || Math.random().toString();
                    if (!nik) { deduped.push(r); continue; }
                    const idx = nikIdx.get(key);
                    if (idx !== undefined) {
                        const existing = deduped[idx];
                        const winner = newerWins(r.emp_code, existing.emp_code);
                        if (String(winner).toUpperCase() === String(r.emp_code || '').toUpperCase()) deduped[idx] = r;
                    } else {
                        nikIdx.set(key, deduped.length);
                        deduped.push(r);
                    }
                }
                rows = deduped;
    ...
    const filtered = useMemo(() => {
        let r = employees;
        if (gang && gang !== 'ALL') {
            const want = String(gang).trim().toUpperCase();
            r = r.filter(e => String(e.gang_code || '').trim().toUpperCase() === want);
        }
    ...
    // Dukung chartType: bila bukan 'ALL', hanya hitung baris yang sesuai jenis pekerjaan.
    const salaryBandTypeCounts = useMemo(() => {
        return salaryDist.counts.map(b => {
            const row = { band: b.band, total: 0, Panen: 0, Transport: 0, Maintenance: 0, Lainnya: 0 };
            for (const e of employees) {
                if (chartType !== 'ALL' && classifyTypeOf(e.gang_code) !== chartType) continue;
                const u = num(e.upah_kotor);
                if (u >= b.min && u < b.max) { const t = classifyTypeOf(e.gang_code); row[t] += 1; row.total += 1; }
            }
            return row;
        });
    }, [salaryDist, employees, chartType]);
    ...
    const global = useMemo(() => {
        ...
        for (const e of globalRoster) {
            ...
            agg.hk += num(e.hari_kerja_efektif ?? (num(e.hari_kerja || e.kehadiran || 0)));
            agg.members.push(e);
            typeMap.set(t, agg);
            pokok += Math.abs(num(e.gaji_pokok));
            lembur += Math.abs(num(e.lembur_jumlah));
        ...
        for (const e of globalRoster) {
            ...
            a.hk += num(e.hari_kerja_efektif ?? (num(e.hari_kerja || e.jumlah_hk || 0)));
    ...
    }, [globalRoster, tonaseMap, chartType]);
                ...
                {!error && division && !loading && employees.length > 0 && (
                    <>
                        ...
                        <Section title="Sebaran Upah Kotor" sub={bandFilter ? `Filter aktif: ${bandFilter} · klik bar lagi untuk reset` : 'Klik bar untuk filter tabel · warna = jenis pekerjaan (panen/transport/maintenance)'}
                            actions={<div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>{['ALL','Panen','Transport','Maintenance'].map(v => <button key={v} onClick={() => setChartType(v)} style={{ padding: '4px 10px', borderRadius: 999, border: `1px solid ${chartType===v ? C.upah : C.border}`, background: chartType===v ? C.upah : C.surface, color: chartType===v ? '#fff' : C.text2, fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>{v==='ALL'?'Semua':v}</button>)} {bandFilter ? <ResetBtn onClick={() => setBandFilter(null)} label="← Semua" /> : null}</div>}>
                            <ResponsiveContainer width="100%" height={240}>
                                <ComposedChart data={salaryBandTypeCounts} margin={{ top: 8, right: 16, bottom: 4, left: 4 }} barGap={4} barCategoryGap={6}>
                                    ...
                                    {TYPE_ORDER.map(t => (
                                        <Bar key={t} dataKey={t} name={t} stackId="a" fill={typeColorMap[t]} minPointSize={6} cursor="pointer" onClick={(d) => { const lab = d?.band || d?.payload?.band; setBandFilter(bf => bf === lab ? null : lab); }} />
                                    ))}
                    ...
                    globalLoading ? (
                        ...
                        <>
                            ...
                            <Section title="Sebaran Gaji Lintas Divisi" sub={globalBand ? `Filter: ${globalBand} · klik bar lagi untuk reset` : 'Klik bar untuk lihat karyawan di rentang itu (semua divisi) · warna = jenis pekerjaan'}
                                actions={<div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>{['ALL','Panen','Transport','Maintenance'].map(v => <button key={v} onClick={() => setChartType(v)} style={{ padding: '4px 10px', borderRadius: 999, border: `1px solid ${chartType===v ? C.upah : C.border}`, background: chartType===v ? C.upah : C.surface, color: chartType===v ? '#fff' : C.text2, fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>{v==='ALL'?'Semua':v}</button>)} {globalBand ? <ResetBtn onClick={() => setGlobalBand(null)} label="← Band Semua" /> : null}</div>}>
                                <ResponsiveContainer width="100%" height={280}>
                                    <ComposedChart data={global.bandTypeCounts} margin={{ top: 8, right: 16, bottom: 4, left: 4 }} barGap={4} barCategoryGap={6}>
                                        <CartesianGrid strokeDasharray="2 4" stroke={C.border} />
                                        <XAxis dataKey="band" tick={{ fontSize: 11, fill: C.text2 }} tickLine={false} axisLine={false} interval={0} />
                                        <YAxis tick={{ fontSize: 12, fill: C.muted }} tickLine={false} axisLine={false} width={32} allowDecimals={false} />
                                        <Tooltip cursor={{ fill: 'rgba(31,111,67,0.06)' }} formatter={(v, n) => [`${v} karyawan`, n]} contentStyle={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 12 }} labelStyle={{ color: C.text, fontWeight: 700 }} />
                                        <Legend wrapperStyle={{ fontSize: 11 }} />
                                        {TYPE_ORDER.map(t => (
                                            <Bar key={t} dataKey={t} name={t} stackId="a" fill={typeColorMap[t]} minPointSize={6} cursor="pointer" onClick={(d) => { const lab = d?.band || d?.payload?.band; setGlobalBand(b => b === lab ? null : lab); }} />
                                        ))}
                                        <Line type="monotone" dataKey="total" name="Total" stroke={C.leafDark} strokeWidth={2} dot={{ r: 3, fill: C.leafDark }} activeDot={{ r: 5 }} isAnimationActive={false} />
                                    </ComposedChart>
                                </ResponsiveContainer>
