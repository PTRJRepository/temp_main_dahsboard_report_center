                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                                ...
                                <tbody>
                                    {(drillDetail.gang_rows || []).map(g => {
                                        const hkEfektif = Math.max(0, (Number(g.total_hk) || 0) - (Number(g.total_cuti_minggu ?? g.total_cuti_minggu) || 0) - (Number(g.total_cuti_nasional) || 0));
                                        const cphk = hkEfektif > 0 ? Number(g.total_upah_kotor) / hkEfektif : 0;
                                        return (
    ...
    <div style={{ ...CARD, marginBottom: 24 }}>
        ...
        <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                ...
                <tbody>
                    {rows.map(g => {
                        const hkEfektif = Math.max(0, (Number(g.total_hk) || 0) - (Number(g.total_cuti_minggu) || 0) - (Number(g.total_cuti_nasional) || 0));
                        const cphk = hkEfektif > 0 ? Number(g.total_upah_kotor) / hkEfektif : 0;
                        return (
                            <tr key={g.gang_code} style={{ borderBottom: `1px solid ${C.border}` }}>
                                <td style={{ padding: '11px 12px', fontWeight: 800 }}>{g.gang_code}</td>
