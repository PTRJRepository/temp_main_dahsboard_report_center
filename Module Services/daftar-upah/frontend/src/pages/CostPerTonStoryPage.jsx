...
import { decomposeCost, costPerTon, costPerHk, hkLibur, productivity, benchmarkMean, deltaPct, heatColor, pivotDivisionSeries, movementBuckets } from '../utils/costPerTonStory.derive';
...
export default function CostPerTonStoryPage() {
    ...
    const breakdown = useMemo(() => {
        ...
        for (const g of gangRows) {
            ...
            if (!agg.has(key)) agg.set(key, { wage: 0, lembur: 0, premi: 0, hk: 0, hkHadir: 0, hc: 0, tonase: tonaseByDiv.get(key) ?? 0 });
            const a = agg.get(key);
            a.wage += Number(g.total_cost) || 0;
            a.lembur += Number(g.total_lembur) || 0;
            a.premi += Number(g.total_premi) || 0;
            a.hk += Number(g.total_hk) || 0;
            // [HK-KEHADIRAN] jumlah HK hadir dari gang (total_hk_efektif = total_hari_kerja).
            a.hkHadir += Number(g.total_hk_efektif ?? 0) || 0;
        ...
        return [...allCodes].map(code => {
            ...
            if (!a) {
                ...
                return src ? { ...src, total_wage: 0, total_ot: 0, total_lembur: 0, total_premi: 0, total_hk: 0, total_hari_kerja: 0, headcount: 0 } : { division_code: code, total_tonase: tonase, total_wage: 0, total_ot: 0, total_lembur: 0, total_premi: 0, total_hk: 0, total_hari_kerja: 0, headcount: 0 };
            }
            const src = base.find(d => String(d.division_code).toUpperCase() === code);
            return { ...(src || { division_code: code }), total_wage: a.wage, total_ot: a.lembur, total_lembur: a.lembur, total_premi: a.premi, total_hk: a.hk, total_hari_kerja: a.hkHadir, headcount: a.hc, total_tonase: tonase };
        });
                ...
                {divsWithUpah.length === 0 ? <EmptyState title="Tonase belum aktif" message="Scatter butuh divisi dengan upah + tonase > 0." /> : (
                    ...
                    <div className="cpts-scatter-wrap">
                        <ResponsiveContainer width="100%" height="100%">
                            <ScatterChart margin={{ top: 20, right: 30, bottom: 30, left: 20 }}>
                                ...
                                <XAxis type="number" dataKey="prod" name="Produktivitas" unit=" t/HK hadir" tick={{ fontSize: 11, fill: '#7C8B80' }} label={{ value: 'Produktivitas (ton / HK hadir)', position: 'insideBottom', offset: -12, fontSize: 12, fill: '#46584C' }} />
                                ...
                                <Tooltip cursor={{ strokeDasharray: '3 3' }} formatter={(v, n) => [n === 'cpt' ? fmtIDR(v) : `${fmtNum(v, 2)} t/HK hadir`, n === 'cpt' ? 'Cost/Ton' : 'Produktivitas (÷ HK hadir)']} contentStyle={{ borderRadius: 10, border: '1px solid #DFE8E0', fontSize: 13 }} />
                                ...
                                <Scatter data={divsWithUpah.map(d => ({ name: d.division_code, prod: productivity(d), cpt: costPerTon(d) })).filter(p => p.cpt != null)} fill="#1F6F43" onClick={(p) => openDrill(p.name)}>
                                    {divsWithUpah.map((d, i) => <Cell key={i} fill={(() => { const p = productivity(d); const c = costPerTon(d); const eff = p != null && meanCpt != null && c < meanCpt && p > (totalTonase / divsWithUpah.reduce((s, x) => s + Number(x.total_hari_kerja ?? x.total_hk ?? 0), 0)); return eff ? '#1F6F43' : (c > meanCpt ? '#B3392E' : '#B45309'); })()} />)}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 8, marginTop: 12, fontSize: 12 }}>
                        {divsWithUpah.map(d => {
                            ...
                            const eff = p != null && meanCpt != null && c < meanCpt && p > (totalTonase / divsWithUpah.reduce((s, x) => s + Number(x.total_hari_kerja ?? x.total_hk ?? 0), 0));
                    ...
                    {/* Uraian HK hadir vs libur + Cost/HK — info tambahan, relevan di sini karena Act 4
                        membahas produktivitas (ton / HK hadir). HK hadir = total_hari_kerja,
                        HK libur = total_hk − hadir (minggu + nasional + izin + sakit + cuti). */}
                    <div style={{ marginTop: 20, background: 'var(--c-surface)', border: '1px solid var(--c-border)', borderRadius: 12, padding: '16px 18px 14px' }}>
                        <div style={{ fontSize: 11, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--c-leafMid)', marginBottom: 4 }}>Detail HK hadir vs HK libur & Cost per HK</div>
                        <div style={{ fontSize: 12.5, color: 'var(--c-text2)', marginBottom: 12 }}>
                            <strong>HK hadir</strong> = hari orang benar-benar masuk kerja (pembagi produktivitas & Cost/HK). <strong>HK libur</strong> = HK total − HK hadir (minggu + nasional + izin + sakit + cuti).
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 10 }}>
                        {divsWithUpah.map(d => {
                            const ton = Number(d.total_tonase) || 0;
                            const hkHadir = Number(d.total_hari_kerja ?? 0) || 0;
                            const hkTotal = Number(d.total_hk ?? 0) || 0;
                            const libur = hkTotal > 0 ? Math.max(0, hkTotal - hkHadir) : null;
                            const cph = costPerHk(d);
                            return (
                                <div key={d.division_code} onClick={() => openDrill(d.division_code)} style={{ background: 'var(--c-surface2)', border: '1px solid var(--c-border)', borderRadius: 8, padding: '10px 12px', cursor: 'pointer' }} title="Klik untuk bedah">
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                                        <strong style={{ color: 'var(--c-ink)', fontSize: 13 }}>{d.division_code}</strong>
                                        <span style={{ fontSize: 10, color: 'var(--c-muted)' }}>ton {fmtNum(ton, 0)}</span>
                                    </div>
                                    <div style={{ fontSize: 11, color: 'var(--c-text2)', lineHeight: 1.6 }}>
                                        <div>HK hadir <strong style={{ color: 'var(--c-upah)' }}>{fmtNum(hkHadir)}</strong> · libur <span style={{ color: 'var(--c-muted)' }}>{libur != null ? fmtNum(libur) : '-'}</span></div>
                                        <div>Cost/HK <strong style={{ color: 'var(--c-leafMid)' }}>{cph != null ? fmtCompact(cph) : '-'}</strong> <span style={{ color: 'var(--c-muted)', fontSize: 10 }}>÷ HK hadir</span></div>
                                    </div>
                                </div>
                            );
                        })}
                ...
                <section className="cpts-print-detail">
                        ...
                        <table className="cpts-print-table">
                            <thead>
                                <tr>
                                    ...
                                    <th style={{ textAlign: 'right' }}>Prod. (t/HK hadir)</th>
                                    <th style={{ textAlign: 'right' }}>HK hadir</th>
                                    <th style={{ textAlign: 'right' }}>HK libur</th>
                            ...
                            <tbody>
                                {[...divsWithUpah].sort((a, b) => costPerTon(b) - costPerTon(a)).map(d => {
                                    ...
                                    const hkHadir = Number(d.total_hari_kerja ?? d.total_hk ?? 0) || 0;
                                    const libur = hkLibur(d);
                                    const cph = costPerHk(d);
                                    return (
                                        <tr key={d.division_code}>
                                            ...
                                            <td style={{ textAlign: 'right' }}>{fmtNum(hkHadir)}</td>
                                            <td style={{ textAlign: 'right', color: '#7C8B80' }}>{libur != null ? fmtNum(libur) : '-'}</td>
                                            <td style={{ textAlign: 'right', fontWeight: 700 }}>{cph != null ? fmtCompact(cph) : '-'}</td>
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td style={{ textAlign: 'right' }}>{totalTonase > 0 ? fmtNum(totalTonase / (divsWithUpah.reduce((s, d) => s + Number(d.total_hari_kerja ?? d.total_hk ?? 0), 0) || 1), 3) : '-'}</td>
                                    <td style={{ textAlign: 'right' }}>{fmtNum(divsWithUpah.reduce((s, d) => s + Number(d.total_hari_kerja ?? d.total_hk ?? 0), 0))}</td>
                                    <td style={{ textAlign: 'right', color: '#7C8B80' }}>{fmtNum(divsWithUpah.reduce((s, d) => s + Number(hkLibur(d) ?? 0), 0))}</td>
                                    <td style={{ textAlign: 'right', fontWeight: 800 }}>{(() => { const totW = divsWithUpah.reduce((s, d) => s + Number(d.total_wage || 0), 0); const totHadir = divsWithUpah.reduce((s, d) => s + Number(d.total_hari_kerja ?? d.total_hk ?? 0), 0); return totHadir > 0 ? fmtCompact(totW / totHadir) : '-'; })()}</td>
                    ...
                    <div className="cpts-print-page">
                        ...
                        <ul className="cpts-print-notes">
                            ...
                            <li><b>Produktivitas</b> = tonase ÷ HK kehadiran (ton per HK hadir). Efisien = produktivitas di atas rata-rata dan cost/ton di bawah rata-rata estate.</li>
                            <li><b>HK hadir / HK libur</b>: HK hadir = hari kehadiran (hari_kerja_efektif = jumlah_hk − cuti_minggu − cuti_nasional). HK libur = HK total − HK hadir (semua selain hadir: libur minggu/nasional + izin + sakit + cuti). <b>Setiap analisis per HK hanya pakai HK hadir sebagai divisor</b> — jangan jumlah_hk.</li>
