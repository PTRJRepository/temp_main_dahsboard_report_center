...
async function fetchEmployeesList(token, filters = {}) {
    // [RETAIN-CONSISTENCY] Default filter status aktif; dedupe nama sama beda empcode ke empcode terbaru
    // HR_EMPLOYEE.Status = '1' menandakan aktif (engine HR). Nilai lain buang.
    try {
        ...
        // Selain caller yang eksplisit minta non-aktif, filter aktif saja
        if (filters.status) params.set('status', filters.status);
        else params.set('status', '1');
        ...
        let data = json.data || [];
        // Double-guard aktif (backend list saat ini belum filter status-nya di SQL)
        data = data.filter(r => String(r.status || '').trim() === '1');
        // Dedupe nama sama beda empcode (employeecode rehire): satu nama → empcode terbaru (C > B > A)
        const nikIdx = new Map();
        const out = [];
        for (const r of data) {
            const nama = String(r.nama || '').trim().toUpperCase();
            if (!nama) { out.push(r); continue; }
            const code = String(r.nik || r.actual_nik || '').trim().toUpperCase();
            const aNik = String(r.actual_nik || '').trim().toUpperCase();
            const dedupeKey = aNik || nama; // rehire → actual_nik (NIK KTP) sama, beda empcode
            const idx = nikIdx.get(dedupeKey);
            if (idx !== undefined) {
                const existing = out[idx];
                const ec = code, en = String(existing.nik || existing.actual_nik || '').trim().toUpperCase();
                if (ec > en) out[idx] = r;
            } else {
                nikIdx.set(dedupeKey, out.length);
                out.push(r);
            }
        }
        return { data: out, dataSource: json.dataSource || 'origin' };
    } catch (err) {
