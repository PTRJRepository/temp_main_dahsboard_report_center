...
export default function ExecutivePayrollPage({ onBack, initialMonth, initialYear, initialDivision }) {
    const { token, user } = useAuth();
    const { isLockedMode } = useReport();
    ...
    // Main Filter State (Header) — terkunci ke divisi kerani bila isLockedMode
    const [selectedFilterDivision, setSelectedFilterDivision] = useState(() => {
        const locked = user?.divisions?.[0] || user?.divisi || initialDivision;
        return isLockedMode && locked ? locked : 'ALL';
    });
