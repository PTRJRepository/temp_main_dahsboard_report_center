...
export default function ManualAdjustmentColumnModal({
    ...
    const [overrideAdCode, setOverrideAdCode] = useState(false);
    const [adCodeOverride, setAdCodeOverride] = useState('');
    const [adDescOverride, setAdDescOverride] = useState('');
    ...
    const handleSubmit = async (event) => {
        ...
        setError('');
        try {
            const defAdCode = resolveAdCode(selectedDefinition);
            const effectiveAdCode = overrideAdCode && adCodeOverride.trim() ? adCodeOverride.trim().toUpperCase() : defAdCode;
            const effectiveTaskDesc = overrideAdCode && adDescOverride.trim() ? adDescOverride.trim().toUpperCase() : selectedDefinition?.task_desc;
            await onSaved?.({
                adjustment_type: adjustmentType,
                adjustment_name: resolvedAdjustmentName,
                ad_code: effectiveAdCode,
                task_code: effectiveAdCode,
                base_task_code: effectiveAdCode,
                task_desc: effectiveTaskDesc,
                loc_code: division && division !== 'ALL' ? division : undefined,
                remarks: buildRemarks({ ad_code: effectiveAdCode, task_desc: effectiveTaskDesc }, resolvedAdjustmentName, 0),
            ...
            >
                ...
                <form onSubmit={handleSubmit} style={{ maxHeight: 'calc(90vh - 86px)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'grid', gap: 14 }}>
                        ...
                        {selectedDefinition && (
                            <div style={{ padding: 12, borderRadius: 12, background: '#fffbeb', border: '1px solid #fde68a', color: '#0f172a' }}>
                                <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                                    <input
                                        type="checkbox"
                                        checked={overrideAdCode}
                                        onChange={(e) => setOverrideAdCode(e.target.checked)}
                                    />
                                ...
                                {overrideAdCode && (
                                    <div style={{ display: 'grid', gap: 8, marginTop: 10 }}>
                                        <div>
                                            <label style={{ fontSize: 12, fontWeight: 700, color: '#92400e', display: 'block', marginBottom: 4 }}>ADCode Override</label>
                                            <input
                                                name="ad_code_override"
                                                value={adCodeOverride}
                                                onChange={(e) => setAdCodeOverride(e.target.value)}
                                                placeholder="contoh: AL9999AB1"
                                                style={{ width: '100%', padding: 9, borderRadius: 8, border: '1px solid #fcd34d', boxSizing: 'border-box' }}
                                            />
                                        </div>
                                        <div>
                                            <label style={{ fontSize: 12, fontWeight: 700, color: '#92400e', display: 'block', marginBottom: 4 }}>AD_DESC Override</label>
                                            <input
                                                name="ad_desc_override"
                                                value={adDescOverride}
                                                onChange={(e) => setAdDescOverride(e.target.value)}
                                                placeholder="contoh: SIMPANG TIGA"
                                                style={{ width: '100%', padding: 9, borderRadius: 8, border: '1px solid #fcd34d', boxSizing: 'border-box' }}
                                            />
                                        </div>
                                    </div>
