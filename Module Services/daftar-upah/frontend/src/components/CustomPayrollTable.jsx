...
import '../styles/CustomPayrollTable.skeuo.css';
...
// Destructure group label constants for use as bare identifiers in column definitions
const { PAJAK, PREMI, POTONGAN_UPAH_KOTOR, POTONGAN_UPAH_BERSIH, UPAH_KOTOR, UPAH_BERSIH,
    PENGGAJIAN, TUNJANGAN, PENDAPATAN_LAINNYA, IDENTITAS, ABSENSI, PANEN } = PAYROLL_HEADER_GROUPS;

// Reverse map: group label → adjustment type
...
// Alias for concise usage in render functions
const toFiniteNumber = toFinitePayrollNumber;

// Default input type for koreksi (blok-based) adjustments
const KOREKSI_DEFAULT_INPUT_TYPE = 'blok';
const KOREKSI_DEFAULT_AD_CODE = '(DE) POTONGAN PREMI';
...
import { ManualCellTrigger, ManualCellDeleteAction, getManualCellTriggerStyle } from './payrollTable/ManualCellActions';
import { ValueSourceCompare } from './payrollTable/ValueSourceCompare';
import {
...
import {
    formatNumber,
    formatDecimal,
    formatNegativeTotalNumber,
    formatBytes,
    clampNumber,
    EMPTY_CELL_STYLE,
    getInitialViewportWidth,
    formatHeaderLabel,
    normalizeValuePriorityMode,
    normalizeFieldKey,
    formatSourceCompareValue,
    STATIC_PREMI_FIELDS,
    STATIC_BRONDOL_FIELD_KEYS,
    STATIC_BRONDOL_LABELS,
    STATIC_POTONGAN_FIELDS,
    normalizeHeaderLabel,
    isBrondolFieldKey,
    isSpsiFieldKey,
    isStaticPremiFieldKey,
    isStaticPotonganFieldKey,
    isStaticBrondolHeader,
    isSpsiLabel,
    isPremiFieldKey,
    DETAIL_TOTAL_MISMATCH_PREMI_NAMES,
    normalizeAdjustmentNameForMismatch,
    buildManualDetailMismatchReason,
    buildIncompleteDetailReason,
    buildManualDetailIssueReason,
    getVisibleManualDetailMismatch,
    getManualDetailValidation,
    isPercobaanGang,
    MANUAL_CELL_DELETE_MARKER,
    buildManualCellDeleteRemarks,
    isManualCellDeleteEdit,
    isAutomaticPayrollKoreksiFieldKey,
    isDynamicGrossDeductionFieldKey,
    isGrossDeductionFieldKey,
    resolveGrossDeductionWithoutAutomaticHk,
    normalizeGrossDeductionForDisplay,
    isPotonganFieldKey,
    isDynamicPotonganFieldKey,
    formatFallbackPremiLabel,
    formatFallbackPotonganLabel,
    parseMetadataObjectValue,
    resolveInitialValuePriorityMode,
    comparePayrollEmployeeRows
} from '../utils/payrollTableHelpers';
...
const CustomPayrollTable = memo(function CustomPayrollTable({
    ...
    // Tema tampilan tabel: 'skeuo' (Estate Ledger, default) | 'classic'
    const [tableTheme, setTableTheme] = useState(() => {
        try {
            if (typeof window === 'undefined') return 'skeuo';
            const stored = window.localStorage.getItem('payroll.table.theme');
            return stored === 'classic' ? 'classic' : 'skeuo';
        } catch {
            return 'skeuo';
        }
    });
    const handleTableThemeChange = (next) => {
        const theme = next === 'classic' ? 'classic' : 'skeuo';
        setTableTheme(theme);
        try {
            window.localStorage.setItem('payroll.table.theme', theme);
        } catch {
            // localStorage tidak tersedia — cukup state sesi
        }
    };
    ...
    const renderManualCellTrigger = ({ label, hasData, displayAmount, hasDbMetadata, hasFallbackMetadata, mismatch, detailValidation, incomplete, pendingDelete, disabled, onClick }) => (
        <ManualCellTrigger
            ...
            hasDbMetadata={hasDbMetadata}
            hasFallbackMetadata={hasFallbackMetadata}
            mismatch={mismatch}
    ...
    const togglesElement = togglePortalTarget ? createPortal(
        <div className={`theme-${tableTheme}`} style={payrollResponsiveVars}>
    ...
    return (
        <div
            className={`payroll-table-shell theme-${tableTheme} mode-${displayMode} ${focusLensEnabled ? 'focus-lens-on' : 'focus-lens-off'}`}
