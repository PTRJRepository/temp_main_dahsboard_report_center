...
export const ReportProvider = ({ children }) => {
    ...
    // Divisi yang diizinkan untuk user terkunci (kerani/prod). Non-admin HANYA boleh lihat divisi ini.
    const allowedLockedDivision = isLockedMode
        ? (externalLockedDiv || prodDivision || user?.divisions?.[0] || user?.divisi || null)
        : null;

    // Guarded setter: user terkunci (kerani/prod) tidak boleh pindah ke divisi lain.
    // Semua halaman yang memakai division dari context otomatis terkunci lewat sini.
    const setDivisionGuarded = React.useCallback((value) => {
        if (allowedLockedDivision && value && String(value).toUpperCase() !== String(allowedLockedDivision).toUpperCase()) {
            console.warn(`[ReportContext] Division locked to ${allowedLockedDivision}, ignoring switch to ${value}`);
            return;
        }
        setDivision(value);
    }, [allowedLockedDivision]);
    ...
    const value = useMemo(() => ({
        month, setMonth,
        year, setYear,
        division, setDivision: setDivisionGuarded,
        gang, setGang,
        ...
        month, year, division, setDivisionGuarded, gang, gangPrefix, gangs, 
