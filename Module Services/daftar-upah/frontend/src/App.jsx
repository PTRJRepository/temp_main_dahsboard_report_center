...
// Eager: needed by OperationalReportWrapper (defined in this file)
...
// Lazy: all page components load on demand per route
...
const DowntimeCommandCenterPage = lazy(() => import('./pages/DowntimeCommandCenterPage'))
const DowntimeAnalyticsPage = lazy(() => import('./pages/DowntimeAnalyticsPage'))
const AssetHealthViewPage = lazy(() => import('./pages/AssetHealthViewPage'))
const TonaseAnalysisReportPage = lazy(() => import('./pages/TonaseAnalysisReportPage'))
const CostPerTonStoryPage = lazy(() => import('./pages/CostPerTonStoryPage'))
const SalaryAnalysisPage = lazy(() => import('./pages/SalaryAnalysisPage'))
...
const ServerMonitor = lazy(() => import('./components/monitor/ServerMonitor'))
...
function AppInner() {
  ...
  useEffect(() => {
    ...
    if (!loading) {
      ...
      // Kerani boleh akses SEMUA halaman analisa/laporan, tapi divisi terkunci ke divisinya
      // (dipaksa oleh ReportContext). Admin-only routes tetap diblok.
      if (isAuthenticated && isKeraniUser) {
        const adminOnlyPaths = ['/seed', '/spreadsheet-sync', '/test'];
        const isAdminOnly = adminOnlyPaths.some(p => currentPath === p || currentPath.startsWith(p));

        if (isAdminOnly) {
          console.log('[App] Kerani user restricted from admin route:', currentPath);
          navigate('/', { replace: true });
        }
      }
    }
      ...
      <Suspense fallback={<LoadingScreen isLoading={true} message="Memuat halaman..." />}>
        <Routes>
          ...
          <Route path="/" element={
            ...
            <Route path="downtime/command-center" element={<SummaryReportWrapper component={DowntimeCommandCenterPage} />} />
            <Route path="downtime/analytics" element={<SummaryReportWrapper component={DowntimeAnalyticsPage} />} />
            <Route path="asset-health" element={<SummaryReportWrapper component={AssetHealthViewPage} />} />
