...
async function getUserFromHeader(headers: Record<string, string | undefined>): Promise<User | null> {
    // [FIX] allowSystemToken: samakan dengan payroll.shared.getUserFromHeader agar
    // internal/system token bisa mengakses /reports/excel (sebelumnya selalu 401
    // untuk pemanggil non-browser meski token valid).
