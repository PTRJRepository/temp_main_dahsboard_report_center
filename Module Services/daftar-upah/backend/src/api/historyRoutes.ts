...
export const historyRoutes = new Elysia({ prefix: "/payroll/history" })
    ...
    .post("/seed/reset", async ({ body, headers, set }) => {
        ...
        const { reason } = body;
        info(CATEGORY, `[HistoryRoutes] Force resetting seeder. Reason: ${reason || 'Not provided'}`);
    ...
    .post("/migrate", async ({ headers, set }) => {
        ...
        info(CATEGORY, `[HistoryRoutes] /migrate: ${results.length - failed.length} ok, ${failed.length} errors`);
    ...
    .post("/seed", async ({ body, headers, set }) => {
        ...
        if (hasValidApiKeyBypass(headers)) {
            ...
            info(CATEGORY, '[HistoryRoutes] API key bypass accepted for seeder');
        }
        ...
        try {
            ...
            info(CATEGORY, `[HistoryRoutes] Seeder result:`, JSON.stringify(result, null, 2));
            ...
            logError(CATEGORY, "[HistoryRoutes] Seed error:", error);
            logError(CATEGORY, "[HistoryRoutes] Error stack:", error.stack);
            set.status = 500;
            return {
                success: false,
    ...
    .get("/", async ({ query, headers, set }) => {
        ...
        try {
            ...
            logError(CATEGORY, "[HistoryRoutes] Get history error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .get("/:history_id", async ({ params, headers, set }) => {
        ...
        try {
            ...
            logError(CATEGORY, "[HistoryRoutes] Get history detail error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .delete("/:history_id", async ({ params, headers, set }) => {
        ...
        try {
            ...
            logError(CATEGORY, "[HistoryRoutes] Delete history error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .post("/:history_id/lock", async ({ params, body, headers, set }) => {
        ...
        try {
            ...
            logError(CATEGORY, "[HistoryRoutes] Lock history error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .get("/audit/trail", async ({ query, headers, set }) => {
        ...
        try {
            ...
            logError(CATEGORY, "[HistoryRoutes] Get audit trail error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .get("/periods/available", async ({ headers, set }) => {
        ...
        try {
            ...
            logError(CATEGORY, "[HistoryRoutes] Get periods error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .get("/current-period", async ({ headers, set }) => {
        try {
            ...
            logError(CATEGORY, "[HistoryRoutes] Current period error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .get("/is-historical/:month/:year", async ({ params, set }) => {
        ...
        try {
            ...
            logError(CATEGORY, "[HistoryRoutes] Is historical check error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .post("/ptkp/update", async ({ body, headers, set }) => {
        ...
        try {
            info(CATEGORY, `[HistoryRoutes] Starting PTKP update for year ${period_year}`);
            ...
            logError(CATEGORY, "[HistoryRoutes] PTKP update error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .post("/ptkp/excel-preview", async ({ body, headers, set }) => {
        ...
        try {
            info(CATEGORY, `[HistoryRoutes] Starting PTKP Excel dry-run for year ${period_year}`);
            ...
            logError(CATEGORY, "[HistoryRoutes] PTKP Excel dry-run error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .get("/ptkp/changelog", async ({ query, headers, set }) => {
        ...
        try {
            ...
            logError(CATEGORY, "[HistoryRoutes] Get PTKP changelog error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .get("/ptkp/preview/:year", async ({ params, headers, set }) => {
        ...
        try {
            ...
            logError(CATEGORY, "[HistoryRoutes] PTKP preview error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .get("/ptkp/:year", async ({ params, headers, set }) => {
        ...
        try {
            ...
            logError(CATEGORY, "[HistoryRoutes] Get PTKP error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .get("/ptkp/employee/:empCode", async ({ params, headers, set }) => {
        ...
        try {
            ...
            logError(CATEGORY, "[HistoryRoutes] Get employee PTKP error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .get("/upah-bersih-detail", async ({ query, headers, set }) => {
        ...
        try {
            ...
            logError(CATEGORY, "[HistoryRoutes] Upah bersih detail error:", error);
            set.status = 500;
            return {
                success: false,
    ...
    .get("/employee/:empCode", async ({ params, set }) => {
        try {
            ...
            logError(CATEGORY, "[HistoryRoutes] Employee historical data error:", error);
            set.status = 500;
            return {
                success: false,
