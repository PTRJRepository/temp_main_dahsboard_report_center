...
import { enforceDivisionScope, filterRowsByAllowedDivision, isKerani } from "../utils/divisionScope";

export const dashboardRoutes = new Elysia({ prefix: "/payroll/dashboard" })
    .derive(async ({ headers }) => {
        // Resolve current user; nullable (tanpa token/API key -> legacy, tidak di-scope).
        const user = await resolveUserFromHeaders(headers, AuthService.getInstance());
        return { currentUser: user };
    })
    .get("/wage-distribution", async ({ query, set, currentUser }) => {
        ...
        try {
            ...
            const divisionCode = enforceDivisionScope(currentUser, query.division_code) || 'ALL';
            const rows = await historyDatabaseService.getWageDistribution(month, year, divisionCode);
            // filter utk kerani: hanya baris divisi izin
            const scopedRows = isKerani(currentUser) ? filterRowsByAllowedDivision(currentUser, rows) : rows;
            ...
            `, [month, year]);
            const tonase: Record<string, number> = {};
            for (const r of tonRows) tonase[r.division_code.trim()] = Number(r.tonase) || 0;
            return { success: true, data: scopedRows, tonase, meta: { count: scopedRows.length, month, year, division_code: divisionCode } };
        } catch (e: any) {
            set.status = 500;
    ...
    .get("/executive-summary", async ({ query, set, currentUser }) => {
        try {
            const month = query.month ? parseInt(query.month) : new Date().getMonth() + 1;
            const year = query.year ? parseInt(query.year) : new Date().getFullYear();
            const scope = ['panen','maintenance','transport','all'].includes(query.scope as string) ? query.scope as string : 'panen';
            ...
            return {
                ...
                data: {
                    trends,
                    breakdown: isKerani(currentUser) ? filterRowsByAllowedDivision(currentUser, breakdown) : breakdown,
                    gangBreakdown: isKerani(currentUser) ? filterRowsByAllowedDivision(currentUser, gangBreakdown) : gangBreakdown,
                    efficiency: isKerani(currentUser) ? filterRowsByAllowedDivision(currentUser, efficiency) : efficiency,
                    productivityTrend,
                    wageSpikes: isKerani(currentUser) ? filterRowsByAllowedDivision(currentUser, wageSpikes) : wageSpikes,
                    kpi,
                    gangScope: scope
                }
            };
    ...
    .get('/division-cost-trend', async ({ query, set, currentUser }) => {
        try {
            const month = query.month ? parseInt(query.month) : new Date().getMonth() + 1;
            const year = query.year ? parseInt(query.year) : new Date().getFullYear();
    ...
    .get('/aggregation/gang-data', async ({ query, set, currentUser }) => {
        try {
            const division = enforceDivisionScope(currentUser, query.division_code) || 'ALL';
            const month = parseInt(query.month);
            const year = parseInt(query.year);
    ...
    .get('/aggregated-gang-data', async ({ query, set, currentUser }) => {
        try {
            const division = enforceDivisionScope(currentUser, query.division_code) || 'ALL';
            const month = parseInt(query.month);
            const year = parseInt(query.year);
    ...
    .get('/premi-analysis', async ({ query, set, currentUser }) => {
        try {
            const month = parseInt(query.month);
            const year = parseInt(query.year);
            const division = enforceDivisionScope(currentUser, query.division_code);
    ...
    .get('/premi-by-division', async ({ query, set, currentUser }) => {
        try {
            const month = parseInt(query.month);
            const year = parseInt(query.year);
            let data = await dashboardService.getPremiByDivision(month, year);
            if (isKerani(currentUser)) data = filterRowsByAllowedDivision(currentUser, data);
            return { success: true, data };
        } catch (e: any) {
            set.status = 500;
            return { success: false, error: e.message };
    ...
    .get('/overtime-analysis', async ({ query, set, currentUser }) => {
        try {
            const month = parseInt(query.month);
            const year = parseInt(query.year);
            const divisionCode = enforceDivisionScope(currentUser, query.division_code);
    ...
    .get('/division-detail-data', async ({ query, set, currentUser }) => {
        try {
            const month = parseInt(query.month);
            const year = parseInt(query.year);
            const divisionCode = enforceDivisionScope(currentUser, query.division_code);
    ...
    .get('/gang-comparison', async ({ query, set, currentUser }) => {
        try {
            const month = parseInt(query.month);
            const year = parseInt(query.year);
            const divisionCode = enforceDivisionScope(currentUser, query.division_code);
            // Tanpa param scope: perilaku lama (semua gang). scope=panen: hanya gang suffix 'H'.
            const scope = ['panen','maintenance','transport','all'].includes(query.scope as string) ? query.scope as string : undefined;
    ...
    .get('/top-bottom-gangs', async ({ query, set, currentUser }) => {
        try {
            const month = parseInt(query.month);
            const year = parseInt(query.year);
            const divisionCode = enforceDivisionScope(currentUser, query.division_code);
    ...
    .get('/gang-history', async ({ query, currentUser }) => {
        const month = parseInt(query.month);
        const year = parseInt(query.year);
    ...
    .get('/all-gangs-trend', async ({ query, currentUser }) => {
        const month = parseInt(query.month);
        const year = parseInt(query.year);
        const divisionCode = enforceDivisionScope(currentUser, query.division_code);
        // Tanpa param scope: perilaku lama (semua gang). scope=panen: hanya gang suffix 'H'.
    ...
    .get('/tonase-analysis-report', async ({ query, set, currentUser }) => {
        try {
            const month = parseInt(query.month);
            const year = parseInt(query.year);
            const divisionCode = enforceDivisionScope(currentUser, query.division_code);
     ...
     */
    .get('/cost-hk-comparison', async ({ query, set, currentUser }) => {
        try {
            const month = parseInt(query.month);
            const year = parseInt(query.year);
            const divisionFilter = enforceDivisionScope(currentUser, query.division_filter) || 'ALL';
