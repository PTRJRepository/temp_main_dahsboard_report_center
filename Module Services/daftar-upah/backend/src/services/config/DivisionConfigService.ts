...
export interface DivisionDefinition {
    ...
    gangPrefix?: string;
    /** HR_GANG LocCode for this division (only set when it differs from code) */
    locCode?: string;
...
export class DivisionConfigService {
    ...
    /**
     * Get the HR_GANG LocCode for a division (accepts any alias or gang code).
     * @example getLocCode('PG1A') -> 'P1A'
     * @example getLocCode('ARB1') -> 'AB1'
     * @example getLocCode('HMC') -> 'WKS_AR'
     */
    public getLocCode(input: string): string {
        if (!input) return input;
        const division = this.getDivision(input);
        return division?.locCode || this.resolveCode(input);
    }
    ...
    /**
     * Unified gang→virtual-division resolution.
     * Replaces the duplicated 3-step pattern found across summaryService,
     * summaryReportService, historyExtractorFormat, EmployeeExtractor, extractQueries.
     *
     * Steps:
     * 1. If sourceLocCode is itself a virtual division, return it.
     * 2. Match gang via source-division-aware pattern (getVirtualDivisionForGang).
     * 3. If no match and gang is not in the caller's gangDivMap, try pattern-only fallback.
     *
     * @param gangCode - Gang code (e.g. 'INF', 'B2N', 'HMC')
     * @param sourceLocCode - LocCode from HR_GANG (e.g. 'P1A', 'AB2')
     * @param gangDesc - Gang description for description-pattern matching
     * @param knownGangDivs - Optional Set of gang codes already mapped to a real division.
     *                         When provided, pattern-only fallback is skipped for these gangs.
     */
    public resolveGangDivision(
        gangCode: string,
        sourceLocCode: string,
        gangDesc?: string,
        knownGangDivs?: Set<string>
    ): string | null {
        if (!gangCode) return null;
        const gc = gangCode.trim().toUpperCase();
        const source = sourceLocCode?.trim().toUpperCase() ?? '';

        // Step 1: source is itself a virtual division
        if (this.isVirtualDivision(source)) {
            return source;
        }

        // Step 2: source-division-aware match
        const matched = this.matchGangToVirtualDivision(gc, gangDesc, source);
        if (matched) return matched;

        // Step 3: pattern-only fallback (skip if gang already has a known real division)
        if (knownGangDivs?.has(gc)) return null;

        return this.matchGangToVirtualDivision(gc, gangDesc, undefined);
    }

    /**
     * Unified division-filter expansion for history/report queries.
     * Single source of truth for the "isVirtual → gang filter, else → division
     * code IN (aliases + source divisions)" pattern previously duplicated across
     * otherIncomesService, otherIncomesThrService, historyExtractorFormat,
     * upahBersihDetailService and dashboardTonaseReport.
     *
     * - Virtual division: gangCodes = DB gangs ∪ aliases (records may store
     *   either gang_code='HMC' or division_code='WKS_AR'); divisionCodes=[canonical].
     * - Real division: divisionCodes = canonical ∪ aliases ∪ source divisions
     *   (with their aliases); gangCodes=[].
     */
    public async expandDivisionFilter(input: string): Promise<{
        divisionCodes: string[];
        gangCodes: string[];
        isVirtual: boolean;
    }> {
        if (!input || input === 'ALL') {
            return { divisionCodes: [], gangCodes: [], isVirtual: false };
        }

        const canonical = this.resolveCode(input);
        const division = this.getDivision(canonical);

        if (!division) {
            return { divisionCodes: [canonical], gangCodes: [], isVirtual: false };
        }

        if (division.type === 'virtual') {
            const gangs = await this.getGangsForDivision(division.code);
            const gangCodes = [...new Set([...gangs.map(g => g.gang_code), ...division.aliases])];
            return { divisionCodes: [division.code], gangCodes, isVirtual: true };
        }

        const codes = new Set<string>();
        for (const sd of this.getSourceDivisions(canonical)) {
            codes.add(sd);
            for (const alias of this.getAliases(sd)) codes.add(alias);
        }
        return { divisionCodes: [...codes], gangCodes: [], isVirtual: false };
    }
