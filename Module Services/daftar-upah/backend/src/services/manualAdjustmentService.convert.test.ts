...
// Patch premiumDefinitionService to use fixture — save originals for restoration
const pProto = PremiumDefinitionService.prototype as any;
const origLoadDefinitions = pProto.loadDefinitions;
const origGetDefinitionsFingerprint = pProto.getDefinitionsFingerprint;
...
afterAll(() => {
    pProto.loadDefinitions = origLoadDefinitions;
    pProto.getDefinitionsFingerprint = origGetDefinitionsFingerprint;
});
...
// Subclass to inject mock db — use Object.create to bypass private constructor
function makeTestService(db: any): ManualAdjustmentService {
    const svc = Object.create(ManualAdjustmentService.prototype) as ManualAdjustmentService;
    (svc as any)._testDb = db;
    return svc;
}
