...
export class AuthService {
    ...
    private constructor() {
        ...
        info(CATEGORY, `[AuthService] Opening SQLite DB: ${dbPath}`);
    ...
    `);
        ...
        if (!admin) {
            info(CATEGORY, "[AuthService] Creating default admin user");
    ...
    public async verifyToken(token: string): Promise<User | null> {
        try {
            info(CATEGORY, `[AuthService] Verifying token. Token starts with: ${token.substring(0, 10)}...`);

            if (Config.DEV_BYPASS_TOKEN && token === Config.DEV_BYPASS_TOKEN) {
                info(CATEGORY, "[AuthService] Dev Bypass Token used. Granting ADMIN access.");
                return {
                    id: 0,
                    username: "dev_admin",
                    email: "dev@admin.com",
            ...
            try {
                ...
                logError(CATEGORY, "[AuthService] Invalid token header:", e);
                return null;
            }
            ...
            if (header.alg === "HS256") {
                ...
                try {
                    ...
                    logError(CATEGORY, "[AuthService] Internal HS256 verification failed:", e);
                    return null;
                }
                ...
                if (Config.AUTH_MODE !== "external") {
                    logError(CATEGORY, `[AuthService] RS256 token ditolak: AUTH_MODE=${Config.AUTH_MODE} (RS256 hanya dipakai saat external/proxy)`);
                    return null;
                }
                try {
                    ...
                    payload = result.payload;
                    info(CATEGORY, "[AuthService] External token verified.");
                } catch (e) {
                    logError(CATEGORY, "[AuthService] External RS256 verification failed:", e);
                    return null;
                }
            } else {
                logError(CATEGORY, `[AuthService] Unsupported token algorithm: ${header.alg}`);
                return null;
            }
            ...
            if (!username) {
                info(CATEGORY, "[AuthService] No username found in token payload");
                // Optional debug log
                // info(CATEGORY, "Payload keys:", Object.keys(payload));
                return null;
            }
            ...
            if ((payload as any).userId && !payload.sub) {
                ...
                divisions = divisions.map(d => divisionConfigService.resolveCode(String(d)));
            ...
            if (user) {
                // info(CATEGORY, `[AuthService] User '${username}' found locally.`);
                const { password_hash, ...safeUser } = user;
                ...
                if (safeUser.divisions && Array.isArray(safeUser.divisions)) {
                    ...
                    info(CATEGORY, `[AuthService] Normalized divisions for user ${username}: ${JSON.stringify(originalDivisions)} -> ${JSON.stringify(safeUser.divisions)}`);
                }
                return safeUser;
            }
            ...
            if (header.alg === "RS256") {
                ...
                if (divisions.length === 0 && role === UserRole.KERANI) {
                    if (username.toLowerCase().startsWith('kerani_')) {
                        ...
                        // info(CATEGORY, `[AuthService] Inferred division '${inferredDiv}' from username '${username}' for KERANI user.`);
                ...
                if (divisions.length === 1 && typeof divisions[0] === 'string') {
                    ...
                    if (first.startsWith("[") && first.endsWith("]")) {
                        try {
                            ...
                            warn(CATEGORY, "[AuthService] Failed to parse stringified divisions array:", first);
                        }
                    }
                }

                if (divisions.length === 0) {
                    warn(CATEGORY, `[AuthService] No divisions found in token for user ${username}. Payload keys: ${Object.keys(payload).join(", ")}`);
                ...
                info(CATEGORY, `[AuthService] Creating transient user from external token. ID: ${externalId}, Role: ${role}`);
                info(CATEGORY, `[AuthService] Transient Divisions extracted: ${JSON.stringify(divisions)}`);
            ...
            logError(CATEGORY, "[AuthService] verifyToken exception:", e);
            return null;
        }
    }
    ...
    private logAudit(userId: number, action: string, details: string) {
        try {
            ...
            logError(CATEGORY, "[AuthService] Failed to log audit:", e);
        }
    }
