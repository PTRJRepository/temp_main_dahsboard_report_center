...
export class EmployeeRepository {
    ...
    public async list(options: any = {}): Promise<{ employees: Employee[]; dataSource: string }> {
        ...
        // [SALARY-ANALYSIS] Status filter bila dikirim oleh frontend (status=1 → aktif saja)
        if (options.status != null && String(options.status).trim() !== '') {
            whereClauses.push("RTRIM(e.Status) = ?");
    ...
    /**
     * Search employees by code/name (used by employee directory search box).
     */
    public async search(q: string, limit: number = 50, division?: string): Promise<{ employees: any[]; dataSource: string }> {
        const term = `%${q.trim()}%`;
        const params: any[] = [term, term];
        let divisionClause = '';
        if (division && division !== 'ALL') {
            const { sql, params: divParams } = divisionConfigService.buildDivisionWhereClause(division, 'e.LocCode');
            divisionClause = sql;
            params.push(...divParams);
        }
        ...
        try {
            const sql = `
                SELECT DISTINCT TOP ${Math.max(1, Math.min(limit, 500))}
                    e.EmpCode, e.NewICNo, e.EmpName, e.Gender, e.LocCode, g.GangCode, p.PayRate,
                    e.Religion, e.Status, e.HREmpType, CONVERT(VARCHAR, e.DOB, 23) AS birth_date
                FROM HR_EMPLOYEE e
                ...
                LEFT JOIN HR_GANG g ON gl.GangCode = g.GangCode
                LEFT JOIN HR_PAYROLL p ON p.EmpCode = e.EmpCode
                WHERE (RTRIM(e.EmpName) LIKE ? OR RTRIM(e.EmpCode) LIKE ?)${divisionClause}
                ORDER BY e.EmpName
            `;
            const rows = await this.db.query<any>(sql, params);
            ...
            logError(CATEGORY, `search failed: ${e.message}`);
            return { employees: [], dataSource: "origin" };
        }
    }

    /**
     * Distinct gang codes present in HR_GANG master.
     */
    public async getAvailableGangs(division?: string): Promise<string[]> {
        const params: any[] = [];
        let divisionClause = '';
        if (division && division !== 'ALL') {
            const { sql, params: divParams } = divisionConfigService.buildDivisionWhereClause(division, 'RTRIM(g.LocCode)');
            divisionClause = ` AND (RTRIM(g.LocCode) IN (${divParams.map(() => '?').join(',')}))`;
            params.push(...divParams);
        }

        try {
            const rows = await this.db.query<{ gang_code: string }>(`
                SELECT DISTINCT RTRIM(g.GangCode) as gang_code
                FROM HR_GANG g
                WHERE RTRIM(g.GangCode) IS NOT NULL AND RTRIM(g.GangCode) != ''${divisionClause}
                ORDER BY gang_code
            `, params);
            return rows.map(r => r.gang_code);
        } catch (e: any) {
            logError(CATEGORY, `getAvailableGangs failed: ${e.message}`);
            return [];
        }
    }

    /**
     * Distinct religions from HR_EMPLOYEE.
     */
    public async getAvailableReligions(): Promise<string[]> {
        try {
            ...
            logError(CATEGORY, `getAvailableReligions failed: ${e.message}`);
            return [];
        }
    }

    /**
     * Distinct employment statuses from HR_EMPLOYEE.
     */
    public async getAvailableStatuses(): Promise<string[]> {
        try {
            ...
            logError(CATEGORY, `getAvailableStatuses failed: ${e.message}`);
            return [];
        }
    }
}
