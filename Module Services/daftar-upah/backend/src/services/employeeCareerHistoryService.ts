...
export class EmployeeCareerHistoryService {
    ...
    async getCareerSummary(identifier: string): Promise<CareerSummary | null> {
        ...
        let firstJoinDate: string | undefined = undefined;
        for (const curr of entriesWithJoinDate) {
            if (!firstJoinDate || new Date(curr.join_date!) < new Date(firstJoinDate)) {
                firstJoinDate = curr.join_date;
            }
        }
