...
export { generateDecemberTaxExcel } from './payroll/tax/taxReportDecemberExcel';
export type { DecemberTaxRow } from './taxReportService';
...
export const generateMonthlyTaxExcel = async (
    ...
    info(CATEGORY, `[generateMonthlyTaxExcel] START - employees=${data?.employees?.length}, year=${year}, month=${month}, division=${division}, gang=${gang}`);
    info(CATEGORY, `[generateMonthlyTaxExcel] First emp keys: ${data?.employees?.[0] ? Object.keys(data.employees[0]).join(', ') : 'NONE'}`);
    info(CATEGORY, `[generateMonthlyTaxExcel] premiKeys parameter: ${premiKeys ? premiKeys.join(', ') : 'NOT PROVIDED'}`);
    ...
    info(CATEGORY, `[generateMonthlyTaxExcel] Discovery: ${empWithPremiDetail} employees have premi_detail, ${empWithPremiDetailEmpty} have empty premi_detail`);
    info(CATEGORY, `[generateMonthlyTaxExcel] Discovered premi keys: ${Array.from(discoveredPremiKeys).join(', ')}`);
    ...
    info(CATEGORY, `[generateMonthlyTaxExcel] DEBUG: data.employees[0] component_metadata type: ${typeof data.employees[0]?.component_metadata}, value: ${JSON.stringify(data.employees[0]?.component_metadata)}`);
    info(CATEGORY, `[generateMonthlyTaxExcel] mainMetaRef keys: ${Object.keys(mainMetaRef).join(', ')}`);
    info(CATEGORY, `[generateMonthlyTaxExcel] First employee has component_metadata: ${!!data.employees[0]?.component_metadata}`);
    if (data.employees[0]?.component_metadata) {
        info(CATEGORY, `[generateMonthlyTaxExcel] component_metadata entries:`, JSON.stringify(data.employees[0].component_metadata));
    }
    ...
    info(CATEGORY, `[generateMonthlyTaxExcel] GL Accounts row: wrote ${glAccCount} cells with DR/CR values`);
    ...
    info(CATEGORY, `[generateMonthlyTaxExcel MAIN] About to loop through ${data.employees.length} employees`);
    ...
    data.employees.forEach((emp, idx) => {
        ...
        if (idx < 3) {
            info(CATEGORY, `[generateMonthlyTaxExcel MAIN] Processing employee ${idx + 1}:`, {
        ...
        if ((emp.no || 1) === 1) {
            info(CATEGORY, `[generateMonthlyTaxExcel MAIN SHEET] First employee premi:`, {
                emp_name: emp.emp_name,
        ...
        if ((emp.no || 1) === 1) {
            info(CATEGORY, `[generateMonthlyTaxExcel MAIN SHEET] First employee THR/bonus:`, {
                emp_name: emp.emp_name,
    ...
    info(CATEGORY, `[generateMonthlyTaxExcel MAIN] Loop complete. Processed ${data.employees.length} employees, last row index: ${currentRowIndex - 1}`);
    ...
    data.employees.forEach((emp, i) => {
        ...
        try {
            ...
            if (i < 2) {
                ...
                info(CATEGORY, `[TaxReport Excel DOM] Processing employee ${i + 1}: Name=${(emp.emp_name || '').substring(0, 20)}, Code=${emp.emp_code}, GP_Ideal=${gpIdeal}, GP_Actual=${gpActual}`);
            }
            ...
            // Debug first employee
            if (i === 0) {
                info(CATEGORY, `[generateMonthlyTaxExcel STD SHEET] First employee:`, {
            ...
            row.getCell(STD_COL_BRUTO).value = { formula: `SUM(${sL(STD_COL_GAJI_POKOK)}${r}:${sL(STD_COL_OTHER_INCOME)}${r})`, result: emp.penghasilan_bruto || 0 };
            ...
            if (i < 2) {
                info(CATEGORY, `[TaxReport Excel] PPH21 mapping for ${emp.emp_name}:`, {
            ...
            logError(CATEGORY, `[TaxReport Excel DOM] CRITICAL ERROR processing employee at index ${i}:`, err);
    ...
    info(CATEGORY, `[generateMonthlyTaxExcel] Calling workbook.xlsx.writeBuffer()...`);
    ...
    info(CATEGORY, `[generateMonthlyTaxExcel] writeBuffer returned ${(buffer as any).byteLength || (buffer as any).length || 0} bytes`);
    if (buffer && ((buffer as any).byteLength > 0 || (buffer as any).length > 0)) {
        info(CATEGORY, `[generateMonthlyTaxExcel] SUCCESS`);
    } else {
        logError(CATEGORY, `[generateMonthlyTaxExcel] WARNING - buffer is empty!`);
    }
