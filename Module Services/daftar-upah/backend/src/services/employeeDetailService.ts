...
export class EmployeeDetailService {
    ...
    /**
     * Batch checkroll: resolve NIK/EmpCode mixes, fetch payroll per division in
     * parallel, then fall back to per-employee checkroll for missed codes.
     * (Extracted from api/employee.ts handleBatchCheckroll.)
     */
    public async getBatchCheckroll(
        empCodesInput: string | string[],
        month: number,
        year: number,
        ...
        data?: any[];
        errors?: any[];
        not_found?: { empCode: string; reason: string }[];
        meta: {
            requested: number;
            successful: number;
            failed: number;
        ...
        const empCodes = Array.isArray(empCodesInput)
            ? empCodesInput.filter(code => typeof code === 'string' && code.trim() !== "")
            : (empCodesInput || "").split(",").filter((code: string) => code.trim() !== "");

        if (empCodes.length === 0) {
            return { success: false, error: "No employee codes provided", meta: { requested: 0, successful: 0, failed: 0, not_found: 0, execution_time_ms: 0 } };
        }
        ...
        try {
            ...
            const nikToResolve = empCodes.filter(code => /^\d{10,}$/.test(code.trim()));
            const codeToResolve = empCodes.filter(code => !/^\d{10,}$/.test(code.trim()));

            const resolvedEmpCodes: string[] = [...codeToResolve];

            if (nikToResolve.length > 0) {
                console.log(`[EmployeeDetailService] [Batch Checkroll] Resolving ${nikToResolve.length} NIKs in single batch query...`);
                try {
                    ...
                    const foundNiks = new Set(nikRows.map(r => r.NewICNo?.trim().toUpperCase()));
                    nikRows.forEach(row => resolvedEmpCodes.push(row.EmpCode.trim()));
                    nikToResolve.forEach(nik => {
                        if (!foundNiks.has(nik.trim().toUpperCase())) {
                            notFound.push({ empCode: nik, reason: "Employee with NIK not found" });
                        }
                    });
                    console.log(`[EmployeeDetailService] [Batch Checkroll] NIK batch resolved ${nikRows.length}/${nikToResolve.length}`);
                } catch (e: any) {
            ...
            try {
                ...
                // Get unique divisions from employees (canonical division codes)
                const divisions = new Set<string>();
                empInfoRows.forEach(row => {
                    if (row.LocCode) {
                        const div = divisionConfigService.resolveCode(row.LocCode.trim());
                        if (div) divisions.add(div);
                    }
                });

                console.log(`[EmployeeDetailService] [Batch Checkroll] Fetched ${empInfoRows.length} employee records, divisions: [${Array.from(divisions).join(", ")}]`);
                ...
                console.log(`[EmployeeDetailService] [Batch Checkroll] Fetched ${allPayrollData.length} payroll rows from ${divisionArray.length} divisions`);
            } catch (e: any) {
                console.error("[EmployeeDetailService] [Batch Checkroll] Failed to fetch payroll data:", e);
            }
            ...
            const missedEmpCodes = actualEmpCodes.filter(empCode =>
                !results.find(res => res.emp_code === empCode.toUpperCase())
            );

            if (missedEmpCodes.length > 0) {
                console.log(`[EmployeeDetailService] [Batch Checkroll] ${missedEmpCodes.length} employees not in batch data, fetching in parallel...`);

                const fallbackPromises = missedEmpCodes.map(async (empCode) => {
                    const normalizedCode = empCode.toUpperCase();
                    try {
                        const individualResult = await this.getEmployeeCheckroll(normalizedCode, month, year, true, useHistoryDb, snapshotVersion);
            ...
            return { success: false, error: e.message, meta: { requested: empCodes.length, successful: results.length, failed: 1, not_found: notFound.length, execution_time_ms: Date.now() - startTime } };
        }
    }
}
