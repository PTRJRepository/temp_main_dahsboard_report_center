...
export class HistorySeederService {
            ...
            await processInBatches({
                ...
                processFn: async (batch) => {
                    for (const r of batch) {
                        ...
                        result.records_inserted['hr_employee'] = (result.records_inserted['hr_employee'] ?? 0) + 1;
                        if (shouldTrackHrAsTotalEmployees) result.total_employees++;
                        processed++;
                    }
    ...
    private async getProfileOverrides(empCodes: string[]): Promise<Map<string, EmployeeProfileOverrideRow>> {
    ...
    private resolveSpsiWithGuard(
        ...
        // ponytail: is_spsi_member diketik boolean, tapi DB lama bisa kirim 0/1 —
        // cek longgar mempertahankan perilaku sebelumnya.
        const spsiRaw: any = override?.is_spsi_member;
        if (spsiRaw === false || spsiRaw === 0) return false;
        if (spsiRaw === true || spsiRaw === 1) return true;
