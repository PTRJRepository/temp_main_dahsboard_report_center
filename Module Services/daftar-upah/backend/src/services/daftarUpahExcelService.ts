...
export async function generateDaftarUpahExcel(
    ...
    for (const emp of sortedRecords) {
        ...
        // ponytail: test expects koreksi in formula — koreksi is negative, adding it reduces gross correctly.
        ...
        const koreksiFormula = HAS_GROSS_KOREKSI ? `+${lPotKoreksi}${r}` : '';
        row.getCell(COL_UPAH_KOTOR).value = {
            // Add koreksi cell (negative value) so formula matches: GP + Tunj + Premi + Koreksi(-)
            formula: `${lGA}${r}+${lTunj}${r}+${lTotalPremi}${r}${otherIncomeFormula}${koreksiFormula}`,
            result: upahKotor
        };
