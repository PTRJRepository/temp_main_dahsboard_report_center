...
export function resolveTaxReportDivisionScope(
    ...
    return {
        ...
        isVirtualDivision: requestedDivisionCode
            ? divisionConfigService.isVirtualDivision(requestedDivisionCode)
            : false
    };
}
...
export function filterTaxReportRows<T extends TaxReportRowLike>(
    ...
    if (scope.isVirtualDivision && scope.requestedDivisionCode) {
        return rows.filter((row) =>
            divisionConfigService.isGangInVirtualDivision(
                row.gang_code || "",
    ...
    return rows.filter((row) => {
        ...
        if (isNumericPrefix) {
            // ponytail: numeric prefix = digit asistensi; helper lives in the
            // legacy wrapper until it is folded into DivisionConfigService.
            return import.meta.require("../services/divisionDefinition").divisionDefinition
                .getAsistensiFromGang(gangCode, scope.fetchDivisionCode) === trimmedPrefix;
        }
